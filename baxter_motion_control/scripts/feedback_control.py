...
class PickAndPlace():
    def __init__(self, shape, desired_position, limb, hover_distance = 0.15, verbose=True):
    ...
    def approach(self,gain):
        ...
        if center:
            diff = np.array(self._desired_position) - np.array(center)
            diff = -1 * gain * diff

            current_pose = self._limb.endpoint_pose()
            ik_pose = Pose()
            ik_pose.position.x = current_pose['position'].x + diff[0]
            ik_pose.position.y = current_pose['position'].y + diff[1]
...
if __name__ == '__main__':
    ...
    hover.position.z = -0.5 + hover_distance
